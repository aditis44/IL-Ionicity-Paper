#!/bin/bash
#SBATCH --job-name=ref_train_eval_optimal_models    # job name
#SBATCH --output="my_output_train_eval_optimal_models_%j" # output file
#SBATCH --error="my_error_train_eval_optimal_models_%j" # error file
...
#train linear L1 model w/optimal hyperparameter value & then do feature importance analysis
modelType='linear_l1'
optimal_hyperparam_val=0.001
directoryToSaveResultsTo=$SLURM_SUBMIT_DIR/optimal_model_example/${molarConductivityOrIonicity}/${modelType}/

python -u train_models.py -molar_conductivity_or_ionicity $molarConductivityOrIonicity -ionic_radii_estimation_method $ionicRadiiEstimationMethod  -directory_to_save_results $directoryToSaveResultsTo -model_type $modelType -compare_to_dummy_regressor $compareToDummyRegressor -do_cross_validation $doCrossValidation -input_features_to_use $input_features_to_use -opt_hyperparameter_val $optimal_hyperparam_val

#train XGBoost model w/optimal hyperparameter value
modelType='xgboost'
optimal_max_depth=13
optimal_n_estimators=860
directoryToSaveResultsTo=$SLURM_SUBMIT_DIR/optimal_model_example/${molarConductivityOrIonicity}/${modelType}/

python -u train_models.py -molar_conductivity_or_ionicity $molarConductivityOrIonicity -ionic_radii_estimation_method $ionicRadiiEstimationMethod  -directory_to_save_results $directoryToSaveResultsTo -model_type $modelType -compare_to_dummy_regressor $compareToDummyRegressor -do_cross_validation $doCrossValidation -input_features_to_use $input_features_to_use -opt_hyperparameter_val $optimal_max_depth -opt_hyperparameter_val2 $optimal_n_estimators
